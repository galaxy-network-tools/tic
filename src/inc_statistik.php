<table align="center">
	...
	<?php
		...
		for ($n = 0; $n < mysqli_num_rows($SQL_Result); $n++) {
			$SQL_Result2 = tic_mysql_query('SELECT COUNT(*) FROM `gn4accounts` WHERE allianz="'.tic_mysql_result($SQL_Result, $n, 'id').'" and ticid="'.$Benutzer['ticid'].'"', $SQL_DBConn);
			$SQL_Row = mysqli_fetch_row($SQL_Result2);
			echo '<tr><td align="left" class="fieldnormaldark">['.tic_mysql_result($SQL_Result, $n, 'tag').'] '.tic_mysql_result($SQL_Result, $n, 'name').'</td><td class="fieldnormallight" align="right">'.$SQL_Row[0].'</td></tr>';
		}
