<?php
	...
	while ($z=mysqli_fetch_row($SQL_Result)) {
		if(!isset($wherenot) || $wherenot == "") $wherenot = "rg=$z[0]";
		else $wherenot.= " OR rg=$z[0]";
	}

	$SQL_Result = tic_mysql_query("SELECT id, zeit FROM `gn4scans` WHERE NOT ($wherenot);", $SQL_DBConn) or die(mysqli_errno()." - ".mysqli_error());

	$scancounter = 0;
	for ($n = 0; $n < mysqli_num_rows($SQL_Result); $n++) {
		$zeit = tic_mysql_result($SQL_Result, $n, 'zeit');
		...
		if($time < time()) {
			$SQL_Result2 = tic_mysql_query('DELETE FROM `gn4scans` WHERE id="'.tic_mysql_result($SQL_Result, $n, 'id').'";', $SQL_DBConn);
			//echo 'DELETE FROM `gn4scans` WHERE id="'.tic_mysql_result($SQL_Result, $n, 'id').'"<br />';
		}
	}
