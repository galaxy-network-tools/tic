...
<?php
  ...
  // Variablen auslesen, damit nur die Berechtigten Atts angezeigt werden
     ...
     for ( $i=0; $i<$SQL_Num; $i++ )  {
      ...
      $lfd = tic_mysql_result($SQL_Result, $i, "lfd");
      ...
      $attdate2 = ConvertDatumToText(tic_mysql_result($SQL_Result, $i, "attdatum"));
      ...
      if ($yesterday > $ATTDate3) {
       ...
       $rg = tic_mysql_result($SQL_Result, $i, 'galaxie');
       $rp = tic_mysql_result($SQL_Result, $i, 'planet');

       $SQL2='select * from `gn4scans` where rg='.tic_mysql_result($SQL_Result, $i, 'galaxie').' and rp='.tic_mysql_result($SQL_Result, $i, 'planet').' and type=0';

       $SQL2_Result = tic_mysql_query($SQL2) or die(tic_mysqli_error(__FILE__,__LINE__));

       if (mysqli_num_rows($SQL2_Result) != 0) {
          $szeit  = tic_mysql_result($SQL2_Result, 0, 'zeit' );
          $sgen   = tic_mysql_result($SQL2_Result, 0, 'gen' );
          $pts    = tic_mysql_result($SQL2_Result, 0, 'pts' );
          $me     = tic_mysql_result($SQL2_Result, 0, 'me' );
          $ke     = tic_mysql_result($SQL2_Result, 0, 'ke' );
          $s      = tic_mysql_result($SQL2_Result, 0, 's' );
          $d      = tic_mysql_result($SQL2_Result, 0, 'd' );
          $a      = tic_mysql_result($SQL2_Result, 0, 'a' );
       ...
       $AttStatus = tic_mysql_result($SQL_Result, $i, "attstatus");
       $freigabe  = tic_mysql_result($SQL_Result, $i, "freigabe");
       ...
       $output2 = InfoText("Ziel wurde erfasst durch Spieler:<br><b>".GetUserInfos(tic_mysql_result($SQL_Result, $i, "id"))." von ".GetAllianzName(tic_mysql_result($SQL_Result, $i, "id")));
...
// Planung
       ...
       echo '<b><center>';
       echo $attdate2;
       echo "<br>".substr( tic_mysql_result($SQL_Result, $i, "attzeit"),0,5)."<BR>";
// Status
       echo $ATTSTATUSINFO[$AttStatus];

       echo '<br>';
       if (tic_mysql_result($SQL_Result, $i, "forall") == 1) {
          echo '</b><font size=1>[ATT_for_all]</font>';
       } elseif (tic_mysql_result($SQL_Result, $i, "formeta") == $Meta) {
          echo '</b><font size=1>[Meta-ATT]</font>';
       } elseif (tic_mysql_result($SQL_Result, $i, "forallianz") == $Ally) {
...
// Eingecheckte Flotten
       ...
       for ( $i2=0; $i2<$SQL2_Num; $i2++ )  {
         ...
         $uid = tic_mysql_result($SQL2_Result, $i2, "id");
// Berücksichtigung der Flottennr.
         $flottenr = tic_mysql_result($SQL2_Result, $i2, "flottenr");
         $ATTETA = '';
         echo '<TR>';
         echo '<TD>';
...
//         echo GetUserInfos($uid);
         ...
         $upts = GetUserPts(tic_mysql_result($SQL2_Result, $i2, "id"));
         if ($ATTETA != '') {
...
// Wenn ID = BenutzerID dann löschen anbieten
         if (tic_mysql_result($SQL2_Result, $i2, "id") == $Benutzer['id']) {
              echo '</TD><TD bgcolor="#'.$ATTSTATUSHTML[$AttStatus].'">';
              echo '<form name="delattflotte" method="post" action="./main.php">';
              echo '<INPUT TYPE="hidden" NAME="lfd" VALUE="'.tic_mysql_result($SQL2_Result, $i2, "lfd").'">';
              echo '<INPUT TYPE="hidden" NAME="id" VALUE="'.tic_mysql_result($SQL2_Result, $i2, "id").'">';
              echo '<INPUT TYPE="hidden" NAME="flotte" VALUE="'.tic_mysql_result($SQL2_Result, $i2, "flottenr").'">';
       ...
       $SQL3='select id from `gn4attflotten` where lfd='.$lfd.' group by id;';
		   $SQL3_Result = tic_mysql_query($SQL3) or die(tic_mysqli_error(__FILE__,__LINE__));
		   $SQL3_Num = mysqli_num_rows($SQL3_Result);
       for ( $i3=0; $i3<$SQL3_Num; $i3++ )  {
          $upts = GetUserPts(tic_mysql_result($SQL3_Result, $i3, "id"));
          $SumPts = $SumPts + $upts;
       }
...
// Info
       ...
       if ($allowedittext == true) {
          ...
          echo '<INPUT TYPE="hidden" NAME="lfd" VALUE="'.tic_mysql_result($SQL_Result, $i, "lfd").'">';
          echo '<INPUT TYPE="hidden" NAME="id" VALUE="'.$Benutzer['id'].'">';
          echo '<center><textarea NAME="info" cols=15 rows='.$rowanzahl.'>'.tic_mysql_result($SQL_Result, $i, "info").'</textarea><br>';
          ...
          echo '</form>';
           ...
           echo tic_mysql_result($SQL_Result, $i, "info");
       }
       echo '</TD>';
// Funktionen
...
// Status änderungen
        ...
        echo '<TD bgcolor="#6490BB"><center>';
              if ($Benutzer['attplaner'] != 0) {
                if (AttPlanerRights( tic_mysql_result($SQL_Result, $i, 'forallianz'),  tic_mysql_result($SQL_Result, $i, 'formeta'),  tic_mysql_result($SQL_Result, $i, 'forall'), $Benutzer['attplaner'], $Benutzer['ticid'], $Benutzer['allianz']) == true) {
// Attplaner funktionen

                  if (tic_mysql_result($SQL_Result, $i, "freigabe") == false) {
                    echo '<a href="./main.php?modul=attplanerlist&lfd='.tic_mysql_result($SQL_Result, $i, "lfd").'&fkt=attfreigabe&action=attplaneradmin2">';
                    echo '<img src="./bilder/attplaner/Ziel_freigabe.gif" width="128" height="25" border="0">';
                    echo '</a><br>';
                  }
                  include './inkl_attplanerfkts.php';
                }
              } elseif ($Benutzer['id'] == tic_mysql_result($SQL_Result, $i, "id") and tic_mysql_result($SQL_Result, $i, "freigabe") == false) {
