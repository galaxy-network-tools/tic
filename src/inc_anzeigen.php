<?php
    ...
    if ($error_code != 0)
        ...
        $zeig_id = tic_mysql_result($SQL_Result, 0, 'id');
        ...
        $zeig_rang = tic_mysql_result($SQL_Result, 0, 'rang');
        $zeig_allianz = tic_mysql_result($SQL_Result, 0, 'allianz');
        $zeig_svs = tic_mysql_result($SQL_Result, 0, 'svs');
        $zeig_sbs = tic_mysql_result($SQL_Result, 0, 'sbs');
        $zeig_umod = tic_mysql_result($SQL_Result, 0, 'umod');
        $zeig_scantyp = tic_mysql_result($SQL_Result, 0, 'scantyp');
        $user_ll = tic_mysql_result($SQL_Result, 0, 'lastlogin');
        $zeit_scaneintraege = tic_mysql_result($SQL_Result, 0, 'scans');
        ...
        if (mysqli_num_rows($SQL_Result2) == 1) {
            $zeig_exen_m = tic_mysql_result($SQL_Result2, 0, 'me');
            $zeig_exen_k = tic_mysql_result($SQL_Result2, 0, 'ke');
        } else {
            $zeig_exen_m = 0;
            $zeig_exen_k = 0;
        }
      ...
      <TD> <font size="-1">
        <?php
                    ...
                    if ($SQL_Num == 0)
                        ...
                        echo '  <TABLE>';
                        for ($n = 0; $n < $SQL_Num; $n++) {
                            $scan = ' <A HREF="./main.php?modul=showgalascans&displaymode=0&xgala='.tic_mysql_result($SQL_Result, $n, 'verteidiger_galaxie').'&xplanet='.tic_mysql_result($SQL_Result, $n, 'verteidiger_planet').'">'.GetScans2($SQL_DBConn, tic_mysql_result($SQL_Result, $n, 'verteidiger_galaxie'), tic_mysql_result($SQL_Result, $n, 'verteidiger_planet')).'</A>';
                            echo '<TR><TD><font size="-1">Name: </td><td><B>'.tic_mysql_result($SQL_Result, $n, 'verteidiger_galaxie').':'.tic_mysql_result($SQL_Result, $n, 'verteidiger_planet').'</B></font></TD>';
                            $disptime = tic_mysql_result($SQL_Result, $n, 'eta') * $Ticks['lange'] - $tick_abzug;
                            $disptime = getime4display( $disptime );
                            echo '<TD><font size="-1">ETA: </td><td><B>'.$disptime.'</B></font></TD>';
                            echo '<TD><font size="-1">Angriffsl&auml;nge: </td><td><B>'.getime4display(tic_mysql_result($SQL_Result, $n, 'flugzeit') * $Ticks['lange']).'</B></font></TD>';
                            echo '<TD><font size="-1">Flotte: </td><td>'.$flottennr[tic_mysql_result($SQL_Result, $n, 'flottennr')].'</font></TD>';
                            echo '<TD><font size="-1">'.$scan.'</font> <td>';
                            /* anstelle von */
//                            if ($Benutzer['rang'] > $Rang_GC || $Benutzer['galaxie'] == $zeig_galaxie) echo ', <A HREF="./main.php?modul=anzeigen&id='.$id.'&action=flotteloeschen&flottenid='.tic_mysql_result($SQL_Result, $n, 'id').'">Löschen</A>, <A HREF="./main.php?modul=flotteaendern&id='.$id.'&flottenid='.tic_mysql_result($SQL_Result, $n, 'id').'">Ändern</A>';
                            /* dies hier */
                            echo '<FORM ACTION="./main.php" METHOD="POST">';
                            echo '<INPUT TYPE="hidden" NAME="modul" VALUE="anzeigen">';
                            echo '<INPUT TYPE="hidden" NAME="id" VALUE="'.$id.'">';
                            echo '<INPUT TYPE="hidden" NAME="action" VALUE="flotteloeschen">';
                            echo '<INPUT TYPE="hidden" NAME="flottenid" VALUE="'.tic_mysql_result($SQL_Result, $n, 'id').'">';
        ...
        <?php
                    ...
                    if ($SQL_Num == 0)
                        ...
                        for ($n = 0; $n < $SQL_Num; $n++) {
                            ...
                            $disptime = getime4display( $disptime );
                            echo '<TD><font size="-1">ETA:</td><td><B>'.$disptime.'</B></font></TD>';
                            echo '<TD><font size="-1">Verteidigungsl&auml;nge:</td><td> <B>'.getime4display(tic_mysql_result($SQL_Result, $n, 'flugzeit') * $Ticks['lange']).'</B></font></TD>';
                            echo '<TD><font size="-1">Flotte:</td><td> '.$flottennr[tic_mysql_result($SQL_Result, $n, 'flottennr')].'</font></TD>';
                            echo '<TD><font size="-1">'.$scan.'</font></td><td>';
//                            if ($Benutzer['rang'] > $Rang_GC || !($Benutzer['rang'] <= $Rang_GC && $Benutzer['galaxie'] != $zeig_galaxie)) echo ', <A HREF="./main.php?modul=anzeigen&id='.$id.'&action=flotteloeschen&flottenid='.tic_mysql_result($SQL_Result, $n, 'id').'">Löschen</A>, <A HREF="./main.php?modul=flotteaendern&id='.$id.'&flottenid='.tic_mysql_result($SQL_Result, $n, 'id').'">&Auml;ndern</A>';
                            echo '<FORM ACTION="./main.php" METHOD="POST">';
                            echo '<INPUT TYPE="hidden" NAME="modul" VALUE="anzeigen">';
                            echo '<INPUT TYPE="hidden" NAME="id" VALUE="'.$id.'">';
                            echo '<INPUT TYPE="hidden" NAME="action" VALUE="flotteloeschen">';
    ...
    <TR>
      <TD> <font size="-1">
        <?php
                    ...
                    $SQL_Num = mysqli_num_rows($SQL_Result);
					for ($x=0;$x<$SQL_Num;$x++){
					$save = tic_mysql_result($SQL_Result,$x,'save');
					...
					if ($SQL_Num == 0) {
                        ...
                        echo '  <TABLE>';
                        for ($n = 0; $n < $SQL_Num; $n++) {
                            $scan = ' <A HREF="./main.php?modul=showgalascans&displaymode=0&xgala='.tic_mysql_result($SQL_Result, $n, 'angreifer_galaxie').'&xplanet='.tic_mysql_result($SQL_Result, $n, 'angreifer_planet').'">'.GetScans2($SQL_DBConn, tic_mysql_result($SQL_Result, $n, 'angreifer_galaxie'), tic_mysql_result($SQL_Result, $n, 'angreifer_planet')).'</A>';
                            echo '<TR><TD><font size="-1">Name: </td><td><B>'.tic_mysql_result($SQL_Result, $n, 'angreifer_galaxie').':'.tic_mysql_result($SQL_Result, $n, 'angreifer_planet').'</B></font></TD>';
                            $disptime = tic_mysql_result($SQL_Result, $n, 'eta') * $Ticks['lange'] - $tick_abzug;
                            $disptime = getime4display( $disptime );
                            echo '<TD><font size="-1">ETA: </td><td><B>'.$disptime.'</B></font></TD>';
                            echo '<TD><font size="-1">Angriffsl&auml;nge: </td><td><B>'.getime4display(tic_mysql_result($SQL_Result, $n, 'flugzeit') * $Ticks['lange']).'</B></font></TD>';
                            echo '<TD><font size="-1">Flotte: </td><td>'.$flottennr[tic_mysql_result($SQL_Result, $n, 'flottennr')].'</font></TD>';
                            echo '<TD><font size="-1">'.$scan.'</font><td>';
//                            if ($Benutzer['rang'] > $Rang_GC || !($Benutzer['rang'] <= $Rang_GC && $Benutzer['galaxie'] != $zeig_galaxie)) echo ', <A HREF="./main.php?modul=anzeigen&id='.$id.'&action=flotteloeschen&flottenid='.tic_mysql_result($SQL_Result, $n, 'id').'">Löschen</A>, <A HREF="./main.php?modul=flotteaendern&id='.$id.'&flottenid='.tic_mysql_result($SQL_Result, $n, 'id').'">Ändern</A>';
                            echo '<FORM ACTION="./main.php" METHOD="POST">';
                            echo '<INPUT TYPE="hidden" NAME="modul" VALUE="anzeigen">';
                            echo '<INPUT TYPE="hidden" NAME="id" VALUE="'.$id.'">';
                            echo '<INPUT TYPE="hidden" NAME="action" VALUE="flotteloeschen">';
      ...
      <TD> <font size="-1">
        <?php
                    ...
                    if ($SQL_Num == 0)
                        ...
                        echo '  <TABLE>';
                        for ($n = 0; $n < $SQL_Num; $n++) {
                            echo '<TR><TD><font size="-1">Name: <B>'.tic_mysql_result($SQL_Result, $n, 'verteidiger_galaxie').':'.tic_mysql_result($SQL_Result, $n, 'verteidiger_planet').'</B></font></TD>';

                            $disptime = tic_mysql_result($SQL_Result, $n, 'eta') * $Ticks['lange'] - $tick_abzug;
                            $disptime = getime4display( $disptime );
                            echo '<TD><font size="-1">ETA: <B>'.$disptime.'</B></font></TD>';

                            echo '<TD><font size="-1">Flotte: '.$flottennr[tic_mysql_result($SQL_Result, $n, 'flottennr')].'</font></TD>';
                            echo '<TD>';
//                            if ($Benutzer['rang'] > $Rang_GC || !($Benutzer['rang'] <= $Rang_GC && $Benutzer['galaxie'] != $zeig_galaxie)) echo '<A HREF="./main.php?modul=anzeigen&id='.$id.'&action=flotteloeschen&flottenid='.tic_mysql_result($SQL_Result, $n, 'id').'">Löschen</A>, <A HREF="./main.php?modul=flotteaendern&id='.$id.'&flottenid='.tic_mysql_result($SQL_Result, $n, 'id').'">Ändern</A></TD>';
                            echo '<FORM ACTION="./main.php" METHOD="POST">';
                            echo '<INPUT TYPE="hidden" NAME="modul" VALUE="anzeigen">';
                            echo '<INPUT TYPE="hidden" NAME="id" VALUE="'.$id.'">';
                            echo '<INPUT TYPE="hidden" NAME="action" VALUE="flotteloeschen">';
