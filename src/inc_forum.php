...
// Themen&uuml;bersicht
                if ($falli == 0)
                    echo '<FONT SIZE=4><B>Allgemeines Forum</B></FONT><BR><FONT SIZE="-1">(Hier k&ouml;nnen sich alle Allianzen gemeinsam besprechen)</FONT><BR><BR>';
                else
                ...
                if ($SQL_Num == 0) {
                    ...
                    for ($n = 0; $n < $SQL_Num; $n++) {
                        $SQL_Result2 = tic_mysql_query('SELECT name FROM `gn4accounts` WHERE  id="'.tic_mysql_result($SQL_Result, $n, 'autorid').'";', $SQL_DBConn) or $error_code = 4;
                        if (mysqli_num_rows($SQL_Result2) != 1)
                            $forum_autor = '<I>Unbekannt</I>';
                        else
                            $forum_autor = '<A HREF="./main.php?modul=anzeigen&id='.tic_mysql_result($SQL_Result, $n, 'autorid').'">'.tic_mysql_result($SQL_Result2, 0, 'name').'</A>';
                        $SQL_Result2 = tic_mysql_query('SELECT autorid, zeit FROM `gn4forum` WHERE belongsto="'.tic_mysql_result($SQL_Result, $n, 'id').'" ORDER BY priority DESC LIMIT 1;', $SQL_DBConn) or $error_code = 4;
                        if (mysqli_num_rows($SQL_Result2) == 1) {
                            $forum_zeit = tic_mysql_result($SQL_Result2, 0, 'zeit');
                            $last_id = tic_mysql_result($SQL_Result2, 0, 'autorid');
                        } else {
                            $forum_zeit = tic_mysql_result($SQL_Result, $n, 'zeit');
                            $last_id = tic_mysql_result($SQL_Result, $n, 'autorid');
                        }
                        ...
                        if (mysqli_num_rows($SQL_Result2) != 1)
                            ...
                            $last_autor = '<A HREF="./main.php?modul=anzeigen&id='.tic_mysql_result($SQL_Result2, 0, 'id').'">'.tic_mysql_result($SQL_Result2, 0, 'name').'</A>';
                        $SQL_Result2 = tic_mysql_query('SELECT COUNT(*) FROM `gn4forum` WHERE belongsto="'.tic_mysql_result($SQL_Result, $n, 'id').'";', $SQL_DBConn) or $error_code = 4;
                        $SQL_Row = mysqli_fetch_row($SQL_Result2);
                        $wichtig = '';
                        if (tic_mysql_result($SQL_Result, $n, 'wichtig') != 0) $wichtig = '<B>(!)</B> ';
                        if ($Benutzer['rang'] > $Rang_GC)
                            echo '  <TR><td BGCOLOR=#'.$htmlstyle['hell'].'><font size="-1">'.$wichtig.'<A HREF="./main.php?modul=forum&faction=show&falli='.$falli.'&ftopic='.tic_mysql_result($SQL_Result, $n, 'id').'">'.tic_mysql_result($SQL_Result, $n, 'topic').'</td></font><td BGCOLOR=#'.$htmlstyle['dunkel'].'><font size="-1"><CENTER>'.ZahlZuText($SQL_Row[0]).'</CENTER></td></font><td BGCOLOR=#'.$htmlstyle['hell'].'><font size="-1"><CENTER>'.ZahlZuText(tic_mysql_result($SQL_Result, $n, 'views')).'</CENTER></td></font><td BGCOLOR=#'.$htmlstyle['dunkel'].'><font size="-1"><CENTER>'.$forum_autor.'</CENTER></td></font><td BGCOLOR=#'.$htmlstyle['hell'].'><font size="-1"><CENTER>'.$forum_zeit.'</CENTER></td></font><td BGCOLOR=#'.$htmlstyle['dunkel'].'><font size="-1"><CENTER>'.$last_autor.'</CENTER></td></font><td BGCOLOR=#'.$htmlstyle['hell'].'><font size="-1"><A HREF="./main.php?modul=forum&faction=delpost&falli='.$falli.'&ftopic='.tic_mysql_result($SQL_Result, $n, 'id').'">L&ouml;schen</A></td></font></TR>';
                        else
...
// Topic anzeigen
                ...
                if ($SQL_Num == 0) {
                    ...
                    $SQL_Result2 = tic_mysql_query('UPDATE `gn4forum` SET views="'.(tic_mysql_result($SQL_Result, 0, 'views') + 1).'" WHERE  id="'.$ftopic.'";', $SQL_DBConn) or $error_code = 4;
                    $SQL_Result2 = tic_mysql_query('SELECT id, ticid, galaxie, planet, name, rang, allianz, umod FROM `gn4accounts` WHERE  id="'.tic_mysql_result($SQL_Result, 0, 'autorid').'";', $SQL_DBConn) or $error_code = 4;
                    if (mysqli_num_rows($SQL_Result2) != 1) {
                        $forum_user['id'] = 0;
                        ...
                        $forum_user['id'] = tic_mysql_result($SQL_Result2, 0, 'id');
                        $forum_user['name'] = tic_mysql_result($SQL_Result2, 0, 'name');
                        $forum_user['rang'] = tic_mysql_result($SQL_Result2, 0, 'rang');
                        $forum_user['allianz'] = tic_mysql_result($SQL_Result2, 0, 'allianz');
                        $forum_user['koords'] = tic_mysql_result($SQL_Result2, 0, 'galaxie').':'.tic_mysql_result($SQL_Result2, 0, 'planet');
                        $forum_user['umod'] = tic_mysql_result($SQL_Result2, 0, 'umod');
                        $forum_user['ticid'] = tic_mysql_result($SQL_Result2, 0, 'ticid');
                        ...
                        $forum_user['tag'] = tic_mysql_result($SQL_Result2, 0, 'tag');
                        $forum_user['aliname'] = tic_mysql_result($SQL_Result2, 0, 'name');
                        $SQL_Result2 = tic_mysql_query('SELECT name FROM `gn4meta` WHERE  id="'.$forum_user['ticid'].'";', $SQL_DBConn) or $error_code = 4;
                        $forum_user['meta'] = tic_mysql_result($SQL_Result2, 0, 'name');
                    }
                    if ($falli == 0)
                    ...
                    echo '  <TR><td BGCOLOR=#333333 WIDTH=80% COLSPAN=2><font size="-1"><FONT COLOR=#FFFFFF SIZE=4><B>'.tic_mysql_result($SQL_Result, 0, 'topic').'</B></FONT></td></font><td BGCOLOR=#333333 WIDTH=20%><font size="-1"><FONT COLOR=#FFFFFF>'.tic_mysql_result($SQL_Result, 0, 'zeit').'</FONT></td></font></TR>';
                    echo '  <TR>';
                    ...
                    if ($Benutzer['rang'] > $Rang_GC || $Benutzer['id'] == $forum_user['id']) echo ', <A HREF="./main.php?modul=forum&faction=update&ftopic='.tic_mysql_result($SQL_Result, 0, 'id').'">Post editieren</A>';
                    if ($Benutzer['rang'] > $Rang_GC) {
                        echo ', <A HREF="./main.php?modul=forum&faction=delpost&falli='.$falli.'&ftopic='.tic_mysql_result($SQL_Result, 0, 'id').'">Post l&ouml;schen</A>';
                        if (tic_mysql_result($SQL_Result, 0, 'wichtig') != 1)
                            echo ', <A HREF="./main.php?modul=forum&faction=setwichtig&falli='.$falli.'&ftopic='.tic_mysql_result($SQL_Result, 0, 'id').'">Wichtig einstellen</A>';
                        else
                            echo ', <A HREF="./main.php?modul=forum&faction=setunwichtig&falli='.$falli.'&ftopic='.tic_mysql_result($SQL_Result, 0, 'id').'">Wichtig ausstellen</A>';
                    }
                    echo '      </td></font>';
                    echo '  </TR>';
                    echo '  <TR>';
                    echo '      <td BGCOLOR=#'.$htmlstyle['hell'].' WIDTH=80% COLSPAN=2 VALIGN="top"><font size="-1">';
                    echo tic_mysql_result($SQL_Result, 0, 'text');
                    echo '      </td></font>';
                    echo '  </TR>';
                    ...
                    if ($SQL_Num > 0) {
                        for ($n = 0; $n < $SQL_Num; $n++) {
                            $SQL_Result2 = tic_mysql_query('SELECT id, ticid, galaxie, planet, name, rang, allianz, umod FROM `gn4accounts` WHERE  id="'.tic_mysql_result($SQL_Result, $n, 'autorid').'";', $SQL_DBConn) or $error_code = 4;
                            if (mysqli_num_rows($SQL_Result2) != 1) {
                                $forum_user['id'] = 0;
                                $forum_user['name'] = '<I>Unbekannt</I>';
                                $forum_user['rang'] = 0;
                            ...
                            if ($forum_user['umod'] != '') echo '<FONT SIZE="-1" COLOR=#'.$htmlstyle['dunkel_blau'].'><B><I>U-Mod aktiviert</I></B></FONT><BR>';
                            echo '      </td></font>';
                            $forum_topic = tic_mysql_result($SQL_Result, $n, 'topic');
                            if ($forum_topic == '') $forum_topic = '<BR>';
                            echo '      <td BGCOLOR=#'.$htmlstyle['dunkel'].' WIDTH=60%><font size="-1">'.$forum_topic.'</td></font>';
                            echo '      <td BGCOLOR=#'.$htmlstyle['dunkel'].' WIDTH=20%><font size="-1">'.tic_mysql_result($SQL_Result, $n, 'zeit').'</td></font>';
                            echo '  </TR>';
                            echo '  <TR>';
                            echo '      <td COLSPAN=2 BGCOLOR=#'.$htmlstyle['dunkel'].'><font size="-1">';
                            echo '          <A HREF="./main.php?modul=anzeigen&id='.$forum_user['id'].'">Profil anschauen</A>';
                            if ($Benutzer['rang'] > $Rang_GC || $Benutzer['id'] == $forum_user['id']) echo ', <A HREF="./main.php?modul=forum&faction=update&ftopic='.tic_mysql_result($SQL_Result, $n, 'id').'">Post editieren</A>';
                            if ($Benutzer['rang'] > $Rang_GC) echo ', <A HREF="./main.php?modul=forum&faction=delpost&falli='.$falli.'&ftopic='.tic_mysql_result($SQL_Result, $n, 'id').'">Post l&ouml;schen</A>';
                            echo '      </td></font>';
                            echo '  </TR>';
                            echo '  <TR>';
                            echo '      <td BGCOLOR=#'.$htmlstyle['hell'].' WIDTH=80% COLSPAN=2 VALIGN="top"><font size="-1">';
                            echo tic_mysql_result($SQL_Result, $n, 'text');
                            echo '      </td></font>';
                            echo '  </TR>';
                        }
                    }
...
// Post Form anzeigen
        if ($ftopic == 0) {
            ...
            if (mysqli_num_rows($SQL_Result) != 1) {
                ...
                $post_title = tic_mysql_result($SQL_Result, 0, 'topic');
...
// Update Form anzeigen
        ...
        if ($post_title != '') {
            ...
            echo '      <INPUT TYPE="hidden" NAME="p_alli" VALUE="'.tic_mysql_result($SQL_Result, 0, 'allianz').'">';
            echo '      <INPUT TYPE="hidden" NAME="p_belongsto" VALUE="'.tic_mysql_result($SQL_Result, 0, 'belongsto').'">';
            ...
            echo '      <TR><td BGCOLOR=#'.$htmlstyle['hell'].'><font size="-1"><TEXTAREA NAME="p_text" ROWS=20 COLS=60>'.str_replace('<BR>', "\n", tic_mysql_result($SQL_Result, 0, 'text')).'</TEXTAREA></td></font></TR>';
            echo '      <TR><td BGCOLOR=#'.$htmlstyle['hell'].'><font size="-1"><INPUT TYPE="submit" VALUE="&auml;ndern"></td></font></TR>';
            echo '  </FORM>';
            echo '</TABLE>';
        }
...
// Forum Post machen
        ...
        if ($access == 1) {
            if (trim($p_topic) == '' || trim($p_text) == '') {
                echo '<FONT COLOR=#FF0000><B>Sie m&uuml;ssen alle Felder ausf&uuml;llen</B></FONT>';
            } else {
                $p_text = str_replace("\n", '<BR>', $p_text);
                ...
                echo '<B>Post hinzugef&uuml;gt</B><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic='.$p_belongsto.'">Zum Thema</A>';
            }
        }
...
// Forumpost l&ouml;schen
        if (!isset($ftopic)) $ftopic = 0;
        if (!isset($falli)) $falli = 0;
        if ($falli > 0 && $falli != $Benutzer['allianz']) {
            echo '<FONT COLOR=#FF0000><B>Sie sind nicht berechtigt diesen Post zu l&ouml;schen</B></FONT><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
        } else {
            if ($ftopic != 0) {
                ...
                echo '<B>Post gel&ouml;scht</B><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$falli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
            } else {
                echo '<FONT COLOR=#FF0000><B>Ung&uuml;ltiger Post</B></FONT><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
            }
        }
...
// Forumpost als wichtig makieren
        ...
        if (!isset($falli)) $falli = 0;
        if ($falli > 0 && $falli != $Benutzer['allianz']) {
            echo '<FONT COLOR=#FF0000><B>Sie sind nicht berechtigt diesen Post wichtig zu makieren</B></FONT><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
        } else {
            if ($ftopic != 0) {
                $SQL_Result = tic_mysql_query('UPDATE `gn4forum` SET wichtig="1" WHERE  id="'.$ftopic.'" AND allianz="'.$falli.'"', $SQL_DBConn);
                echo '<B>Post wichtig makiert</B><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
            } else {
                echo '<FONT COLOR=#FF0000><B>Ung&uuml;ltiger Post</B></FONT><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
            }
        }
// Forumpost als unwichtig makieren
        ...
        if (!isset($falli)) $falli = 0;
        if ($falli > 0 && $falli != $Benutzer['allianz']) {
            echo '<FONT COLOR=#FF0000><B>Sie sind nicht berechtigt diesen Post als unwichtig zu makieren</B></FONT><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
        } else {
            if ($ftopic != 0) {
                $SQL_Result = tic_mysql_query('UPDATE `gn4forum` SET wichtig="0" WHERE  id="'.$ftopic.'" AND allianz="'.$falli.'"', $SQL_DBConn);
                echo '<B>Post unwichtig makiert</B><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
            } else {
                echo '<FONT COLOR=#FF0000><B>Ung&uuml;ltiger Post</B></FONT><BR><A HREF="./main.php?modul=forum&faction=show&falli='.$p_alli.'&ftopic=0">Zur Themen&uuml;bersicht</A>';
            }
        }
