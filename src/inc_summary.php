...
<?php
	...
	$SQL_Result_Alli = tic_mysql_query($SQL_Query) or die(tic_mysqli_error(__FILE__,__LINE__));
	$alli_anzahl=mysqli_num_rows($SQL_Result_Alli);
	if ($alli_anzahl<8) {
		...
		for($b=0; $b<$alli_anzahl; $b++) {
			$alliid = tic_mysql_result($SQL_Result_Alli, $b, 'id');
			$metaid = tic_mysql_result($SQL_Result_Alli, $b, 'ticid');
			$incs_offen = 0;
			$incs_overtime = 0;
			$incs_safe = 0;
			...
			$SQL_Result_alli_inc_fleets = tic_mysql_query($SQL_Query) or die(tic_mysqli_error(__FILE__,__LINE__));
	
			while($inc = mysqli_fetch_assoc($SQL_Result_alli_inc_fleets)) {
				if ( $inc['save'] != 0 )
			...
			mysqli_free_result($SQL_Result_alli_inc_fleets);
			...
			if (mysqli_num_rows($SQL_Result_alli_user_online) > 0)
				for ($n = 0; $n < mysqli_num_rows($SQL_Result_alli_user_online); $n++) {
					$rang = tic_mysql_result($SQL_Result_alli_user_online, $n, 'rang');
					$out_online_names .= tic_mysql_result($SQL_Result_alli_user_online, $n, 'galaxie').":".tic_mysql_result($SQL_Result_alli_user_online, $n, 'planet')." [".$AllianzTag[tic_mysql_result($SQL_Result_alli_user_online, $n, 'allianz')]."] ".($rang == $Rang_STechniker?"&lt;font color=#ff0000&gt;":($rang == $Rang_Techniker?"&lt;font color=#0000ff&gt;":"")).($rang > $Rang_GC?"&lt;b&gt;":"").tic_mysql_result($SQL_Result_alli_user_online, $n, 'name').($rang > $Rang_GC?"&lt;/b&gt;":"").($rang >= $Rang_Techniker?"&lt;/font&gt;":"")."&lt;br&gt;";
				}
			else
			...
			$out_online	 .= "		<td align=\"center\" onmouseover=\"return overlib('".$out_online_names."');\" onmouseout=\"return nd();\">&nbsp;".mysqli_num_rows($SQL_Result_alli_user_online)." / ".tic_mysql_result($SQL_Result_alli_user, 0, 'count(*)')."&nbsp;</td>\n";
	
			mysqli_free_result($SQL_Result_alli_user);
			mysqli_free_result($SQL_Result_alli_user_online);
		}
		mysqli_free_result($SQL_Result_Alli);
	}else{
		...
		$SQL_Result_Metas = tic_mysql_query($SQL_Query) or die(tic_mysqli_error(__FILE__,__LINE__));
		$manzahl = mysqli_num_rows($SQL_Result_Metas);
	
		$tsec = $Ticks['lange']*60;
		$time_now = ((int)(time()/($tsec)))*($tsec);
	
		for($b=0; $b<$manzahl; $b++) {
			$mname = tic_mysql_result($SQL_Result_Metas, $b, 'value');
			$mticid = tic_mysql_result($SQL_Result_Metas, $b, 'ticid');
			$incs_offen = 0;
			$incs_overtime = 0;
			$incs_safe = 0;
