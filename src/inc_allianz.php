...
<?
	...
	for ($n = 0; $n < $SQL_Num; $n++) {
		$zusatz = 'normal';
		$lastlogin = tic_mysql_result($SQL_Result, $n, 'lastlogin');
		...
		if (tic_mysql_result($SQL_Result, $n, 'id') == $Benutzer['id']) $zusatz = 'myself';

		if(tic_mysql_result($SQL_Result, $n, 'me') != "")
			$exen_m = tic_mysql_result($SQL_Result, $n, 'me');
		else
			$exen_m = "0";
		if(tic_mysql_result($SQL_Result, $n, 'ke') != "")
			$exen_k = tic_mysql_result($SQL_Result, $n, 'ke');
		else
			$exen_k = "0";

		$koord_g = tic_mysql_result($SQL_Result, $n, 'galaxie');
		$koord_p = tic_mysql_result($SQL_Result, $n, 'planet');
		...
		echo "			<td class=\"field".$zusatz."dark\"><img src=\"".$RangImage[tic_mysql_result($SQL_Result, $n, 'rang')]."\" width=\"20\" height=\"20\" border=\"0\" alt=\"".$RangName[tic_mysql_result($SQL_Result, $n, 'rang')]."\" title=\"".$RangName[tic_mysql_result($SQL_Result, $n, 'rang')]."\" align=\"middle\" /> <a href=\"main.php?modul=anzeigen&amp;id=".tic_mysql_result($SQL_Result, $n, 'id')."\">".tic_mysql_result($SQL_Result, $n, 'name')."</a></td>\n";
		...
		echo "			<td class=\"field".$zusatz."light\" align=\"right\">".ZahlZuText(tic_mysql_result($SQL_Result, $n, 'svs'))."</td>\n";
		echo "			<td class=\"field".$zusatz."dark\" align=\"right\">".ZahlZuText(tic_mysql_result($SQL_Result, $n, 'sbs'))."</td>\n";
		echo "			<td class=\"field".$zusatz."light\">".$ScanTyp[tic_mysql_result($SQL_Result, $n, 'scantyp')]."</td>\n";
		...
		$gesamt_sbs = $gesamt_sbs + tic_mysql_result($SQL_Result, $n, 'sbs');
		if (tic_mysql_result($SQL_Result, $n, 'sbs') > 0) $sb_spieler++;
	}
	if ($sb_spieler > 0)
